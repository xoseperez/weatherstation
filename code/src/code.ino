...
void bmeSetup() {
    ...
    #ifdef DEBUG
    if (!status) {
        Serial.print("[SENSOR] Could not find a valid BME280 sensor, check wiring, address, sensor ID!\n");
        Serial.print("[SENSOR] SensorID was: 0x"); Serial.println(bme.sensorID(),16);
        Serial.print("[SENSOR]    ID of 0xFF probably means a bad address, a BMP 180 or BMP 085\n");
        Serial.print("[SENSOR]    ID of 0x56-0x58 represents a BMP 280\n");
        Serial.print("[SENSOR]    ID of 0x60 represents a BME 280.\n");
        Serial.print("[SENSOR]    ID of 0x61 represents a BME 680.\n");
        while (1) delay(10);
    }
    #endif
